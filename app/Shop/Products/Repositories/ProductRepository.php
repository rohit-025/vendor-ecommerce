...
class ProductRepository extends BaseRepository implements ProductRepositoryInterface
{
    ...
    public function updateProduct(array $data) : bool
    {
        ...
        try {
            $id = $this->model->id;
            return $this->model->find($id)->update($filtered);
        } catch (QueryException $e) {
    ...
    {
        return $this->uploadOne($file,'products');
    }
    ...
    public function saveProductImages(Collection $collection)
    {
        $collection->each(function (UploadedFile $file) {
            $filename = $this->uploadOne($file,'products');
    ...
    {
        $latestId = $this->model->select('id')->latest()->first();

        return (strtoupper(substr($name, 0, 3)).((!!$latestId) ? ($latestId->id+1) : 1));
    }
