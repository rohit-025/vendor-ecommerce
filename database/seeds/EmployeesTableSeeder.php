...
class EmployeesTableSeeder extends Seeder
{
    public function run()
    {
        ...
        $employee = factory(Employee::class)->create([
            'email' => 'vendor@doe.com'
        ]);

        $vendor = factory(Role::class)->create([
            'name' => 'vendor',
            'display_name' => 'Vendor'
        ]);

        $roleVendorRepo = new RoleRepository($vendor);
        $roleVendorRepo->attachToPermission($createProductPerm);
        $roleVendorRepo->attachToPermission($viewProductPerm);
        $roleVendorRepo->attachToPermission($updateProductPerm);
        $roleVendorRepo->attachToPermission($deleteProductPerm);
        
        $employee->roles()->save($vendor);
    }
}
