    ...
    Route::namespace('Admin')->group(function () {
        Route::group(['middleware' => ['role:admin|superadmin|clerk, guard:employee']], function () {
            ...
            Route::namespace('Offers')->group(function () {
                Route::get('offers', 'OfferController@index')->name('offers.list');
                Route::get('offer', 'OfferController@create')->name('offers.create');
                Route::post('offer','OfferController@store')->name('offers.store');
                Route::get('product-by-category/{category_id}','OfferController@productByCategory');
                Route::post('/offer/add-products','OfferController@addProduct');
            });
        ...
        Route::post('/tax', 'Settings\SettingsController@updateTax')->name('tax');

        Route::post('/shipping', 'Settings\SettingsController@updateShipping')->name('shipping');

        Route::post('/sizes', 'Settings\SettingsController@updateSizes')->name('sizes');
    });
});
...
Route::namespace('Front')->group(function () {
    Route::get('search-auto','ProductController@searchAuto');
    ...
    Route::group(['middleware' => ['auth', 'web']], function () {
        ...
        // Route::get('addresses', 'AccountsController@addressList')->name('addressList');
    ...
    Route::get('/shop/{slug}', 'ShopController@shop');


    Route::get("/product-by-id/{id}",'ProductController@productById');

    Route::post('stay-in-touch', 'StayInTouchController@subscribe')->name('subscribe');

    Route::get('/notify/{productId}','StayInTouchController@notify')->middleware('auth');

    Route::get('/blogs','BlogController@index');

    Route::get('/blogs/{slug}','BlogController@blogBySlug');
...
Route::group(['prefix' => 'vendor'],function(){
    Route::namespace('Admin')->group(function () {
        Route::namespace('Vendor')->group(function () {
            ...
            Route::post('/register','VendorController@store')->name('registerVendor');
        });
    });
    });
